...
public class ScheduleTablePane extends JPanel {
	...
	public List<ClassInfo> getClassOfOneTeacher(String card) {
		Response response = ResponseUtils.getResponseByHash(
				new Request(App.connectionToServer, null, "tech.zxuuu.server.teaching.ClassSelectGUI.getClassOfOneTeacher", new Object[] { card }).send()
		);

		if (response == null) {
			System.out.println("Error: Server response is null");
			return null;
		}

		return response.getListReturn(ClassInfo.class);
	}
	...
	public ScheduleTablePane() {
		...
		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 6; j++) {
				...
				labels[index].setBounds(j * 148, i * 103, 200, 180);
//				此处修改组件大小和课程！！！
