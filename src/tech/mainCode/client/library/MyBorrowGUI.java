...
public class MyBorrowGUI extends JDialog {
	...
	public MyBorrowGUI() {
		...
		JButton btnReturn = new RoundedButton("还 书",10);
		btnReturn.setBounds(104, 334, 120, 40);
		btnReturn.setFont(new Font("微软雅黑", Font.PLAIN, 22));
		btnReturn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				int result = ResponseUtils
						.getResponseByHash(
								new Request(App.connectionToServer, App.session, "tech.mainCode.server.library.BookServer.returnBook",
		...
		JButton btnRenew = new RoundedButton("续 借",10);
		btnRenew.setFont(new Font("微软雅黑", Font.PLAIN, 22));
		btnRenew.setBounds(341, 334, 120, 40);
		btnRenew.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				int result = ResponseUtils
						.getResponseByHash(new Request(App.connectionToServer, App.session,
								"tech.mainCode.server.library.BookServer.renewBook", new Object[] { txtISBN.getText() }).send())
						.getReturn(Integer.class);
				if (result == 0)
