...
import tech.mainCode.util.JSONUtils;
...
//	}
	public <T> List<T> getListReturn(Class<T> elementClazz) {
		...
		JSONArray tempParam = (JSONArray) this.param;
		System.out.println("tempParam: " + tempParam.toJSONString());

		List<Object> unparsedList = JSON.parseArray(tempParam.toJSONString());
		System.out.println("unparsedList: " + unparsedList);
		...
		if (JSONUtils.isBasicClass(elementClazz)) {
			for (Object obj : unparsedList) {
				System.out.println("Basic class object: " + obj);
				processedList.add(elementClazz.cast(obj));
			}
		} else {
			// 处理嵌套类型
			for (Object obj : unparsedList) {
				...
				T parsedObject = JSON.parseObject(jsonObject.toJSONString(), elementClazz);
				System.out.println("parsedObject: " + parsedObject);
				processedList.add(parsedObject);
			}
		}
