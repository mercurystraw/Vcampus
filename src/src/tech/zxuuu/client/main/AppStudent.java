...
import tech.zxuuu.client.main.NoticeBlock;
import tech.zxuuu.client.main.PostBlock;
...
public class AppStudent extends JFrame {
	...
	public AppStudent() {
		 ...
		 * 新增，初始化控件内容
		 */
		lblName.setText(tech.zxuuu.client.main.App.session.getStudent().getName());
		lblAcademy.setText(tech.zxuuu.client.main.App.session.getStudent().getAcademy() + " - "
				+ OtherUtils.getAcademyByNumber(tech.zxuuu.client.main.App.session.getStudent().getAcademy()));
		lblBalance.setText(String.format("%.2f", tech.zxuuu.client.main.App.session.getStudent().getBalance()));
		lblStudentNumber.setText(tech.zxuuu.client.main.App.session.getStudent().getStudentNumber());
		lblCardNumber.setText(tech.zxuuu.client.main.App.session.getStudent().getCardNumber());
		List<Book> tempList = ResponseUtils.getResponseByHash(
						new Request(tech.zxuuu.client.main.App.connectionToServer, null, "tech.zxuuu.server.library.Addons.getBorrowedBookList",
								new Object[] { tech.zxuuu.client.main.App.session.getStudent().getCardNumber() }).send())
				.getListReturn(Book.class);
		if (tempList == null || tempList.size() == 0) {
			lblBookLend.setText("0 本");
		} else {
		...
		btnRecharge.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Boolean result = ResponseUtils
						.getResponseByHash(
								(new Request(tech.zxuuu.client.main.App.connectionToServer, null, "tech.zxuuu.server.main.UtilsApi.chargeCardBalance",
										new Object[] { tech.zxuuu.client.main.App.session.getStudent().getCardNumber(), Integer.parseInt(money) }).send()))
						.getReturn(Boolean.class);
		...
		Integer today = OtherUtils.getDay(ResponseUtils.getResponseByHash(
						new Request(tech.zxuuu.client.main.App.connectionToServer, null, "tech.zxuuu.server.main.UtilsApi.getTrustedUnixTimeStamp", null)
								.send())
				.getReturn(Long.class));
		...
		String selectedClass = ResponseUtils.getResponseByHash(
						new Request(tech.zxuuu.client.main.App.connectionToServer, null, "tech.zxuuu.server.teaching.ClassSelectGUI.getClassSelection",
								new Object[] { tech.zxuuu.client.main.App.session.getStudent() }).send())
				.getReturn(String.class);
		...
		if (selectedClass != null && !selectedClass.equals("")) {
			...
			for (int i = 0; i < course.length; i++) {
				if (Integer.valueOf(course[i].charAt(6) - 48) == today) {
					ClassInfo cla = ResponseUtils
							.getResponseByHash(new Request(tech.zxuuu.client.main.App.connectionToServer, null,
									"tech.zxuuu.server.teaching.ClassSelectGUI.getOneClass", new Object[] { course[i] }).send())
							.getReturn(ClassInfo.class);
		...
		cardnumber.setText(tech.zxuuu.client.main.App.session.getStudent().getCardNumber());
		...
		List<NoticeInfo> noticeInfos = ResponseUtils
				.getResponseByHash(
						new Request(tech.zxuuu.client.main.App.connectionToServer, null, "tech.zxuuu.server.main.UtilsApi.getTop4Notice", null).send())
				.getListReturn(NoticeInfo.class);
		...
		if (postInfos.size() > 0) {
			tech.zxuuu.client.main.PostBlock postBlock1 = new tech.zxuuu.client.main.PostBlock(postInfos.get(0).getId(), postInfos.get(0).getContent(),
					quickFormatDate(postInfos.get(0).getDate()), postInfos.get(0).getUser_id(), postInfos.get(0).getThumbup());
			postBlock1.setBounds(57, 422, 660, 80); // 调整位置
			contentPane.add(postBlock1);
		}
		...
		if (postInfos.size() > 1) {
			tech.zxuuu.client.main.PostBlock postBlock2 = new tech.zxuuu.client.main.PostBlock(postInfos.get(1).getId(), postInfos.get(1).getContent(),
					quickFormatDate(postInfos.get(1).getDate()), postInfos.get(1).getUser_id(), postInfos.get(1).getThumbup());
			postBlock2.setBounds(57, 512, 660, 80); // 调整位置
			contentPane.add(postBlock2);
		}
		...
		if (postInfos.size() > 2) {
			tech.zxuuu.client.main.PostBlock postBlock3 = new PostBlock(postInfos.get(2).getId(), postInfos.get(2).getContent(),
					quickFormatDate(postInfos.get(2).getDate()), postInfos.get(2).getUser_id(), postInfos.get(2).getThumbup());
			postBlock3.setBounds(57, 602, 660, 80); // 调整位置
			contentPane.add(postBlock3);
		}
