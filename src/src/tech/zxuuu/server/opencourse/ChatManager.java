...
public class ChatManager extends Thread {
	...
	private ChatManager() {
		...
		userLists = new HashMap<Integer, Vector<tech.zxuuu.server.opencourse.ChatSocket>>();
		userLists.clear();
		userLists.put(-1, new Vector<tech.zxuuu.server.opencourse.ChatSocket>());
	}

	private ServerSocket serverSocket;
	private Map<Integer, Vector<tech.zxuuu.server.opencourse.ChatSocket>> userLists;

	private void broadcast(String str, tech.zxuuu.server.opencourse.ChatSocket speaker) {
		String text = "";
		...
		for (tech.zxuuu.server.opencourse.ChatSocket user : userLists.get(key)) {
			user.write(text);
		}
	}

	private void receive(String str, tech.zxuuu.server.opencourse.ChatSocket speaker) {
		if (speaker.getCourseId() < 0 || speaker.getUserType() == null || speaker.getName().equals("")) {
			...
			if (!userLists.containsKey(key)) {
				userLists.put(key, new Vector<tech.zxuuu.server.opencourse.ChatSocket>());
			}
	...
	public void run() {
		try {
			...
			while (true) {
				userLists.get(-1).add(
						new tech.zxuuu.server.opencourse.ChatSocket(serverSocket.accept(), (String str, ChatSocket chatSocket) -> receive(str, chatSocket)));
